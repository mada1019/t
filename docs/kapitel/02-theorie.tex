...
Die Modellprädiktive Regelung ist eine mathematische Methodik zur Regelung von Prozessen oder technischen Systemen hinsichtlich gewählter Optimierungskriterien. Hierzu bedient sie sich mathematischer Modelle des zu steuernden Prozesses sowie einem Teilbereich der Mathematik, der Optimalsteuerung. Zur Regelung eines Prozesses werden wiederholt Optimalsteuerungsprobleme gelöst, wodurch sich besondere Anforderungen an das Modell ergeben \cite[S.~10]{di14}. Daher erfolgt zunächst eine kurze Einführung in die Optimalsteuerung bevor die Modellprädiktive Regelung erläutert wird.

Die Optimalsteuerung ist ebenfalls ein Teilgebiet der Mathematik und dient dazu, dynamische Systeme optimal zu steuern hinsichtlich spezifizierter Kriterien zur Optimierung. Es sollen also Steuersignale bestimmt werden, sodass das System eventuell vorgegebene physikalischen und nutzerdefinierten Rahmen- sowie Randbedingungen erfüllt und zudem ein Leistungskriterium minimiert beziehungsweise maximiert wird \cite[S.~3f.]{ki04}.


Hohe Modellgüte bei gleichzeitig geringer Komplexität und Steigkeit

Einerseits sollen Modelle reales Verhalten beschreiben, andereseits

das Verhalten des Systems in der Zukunft -- also einer immer gleich weit in die Zukunft hineinreichenden Periode -- zu beschreiben. Hierzu bedient \acrlong{mpc} sich der Kenntnis des aktuellen Zustandes und eines physikalischen-mathematischen Modells des Systems, um dessen zukünftiges Verhalten \Gun vorherzusagen \Gob bzw. abzubilden. Des Weiteren wird versucht das Verhalten des Systems mit minimalem Aufwand zu beeinflussen, um einem eigens- oder vordefinierten Zielkriterium zu folgen \acrlong{bzw} diesem zu entsprechen.

Einerseits sollten die Modelle das reale Verhalten der Komponenten möglichst genau beschreiben, anderseits darf die Komple- xität der Modelle nicht zu hoch sein, um die Rechenzeit möglichst gering zu halten und damit die Steuerung im laufenden Betrieb zu ermöglichen. Die Modellprädiktiven Regelung basiertEinleitung 2  auf der Theorie der Optimalen Steuerung (engl. Optimal Control Theory, kurz OPC). Beim Einsatz von MPC wird ein Optimalsteuerungsproblem im laufenden Betrieb der Anlage im- mer wieder neu gelöst. Die in diesem Fall angewendete Methode zur Lösung des Optimals- teuerungsproblems bedarf der Ermittlung von Ableitungen der Modellgleichungen. Daher stellt die Stetigkeit der Modelle eine weitere essentielle Bedingung dar.
 gen (ODE model) beschrieben wird, dessen Zustandsvariablen der Vektor g repräsentiert . . beinhaltet die Steuerfunktionen (controls) für das Zeitintervall   = 0 bis   = . Das Leis- tungskriterium wird durch eine zu minimierende Kostenfunktion ausgedrückt und besteht aus dem Lagrange (1)- und dem Meyer-Term (2). Ersterer Term beschreibt die Kosten, die über das gesamte Zeitintervall anfallen, letzterer bezieht sich ausschließlich auf die Endkosten zumwobei der zu steuernde Prozess hier durch ein Modell aus gewöhnlichen Differentialgleichun- 1 Die hier dargelegten Form eines Optimalsteuerungsproblems gilt für Modelle die aus gewöhnlichen Differenti- algleichungen bestehen. Es existiert auch eine Form für Modelle die aus Differential-algebraischen Gleichun- gen aufgebaut sind.
Optimale Steuerung und Modellprädiktive Regelung 5Zeitpunkt . Durch die path constraints wird der zulässige Wertebereich für g und . über das Optimierungsintervall festgelegt. Unter Verwendung von terminal constraints erfolgt ledig- lich die Einschränkung des Wertebereichs für g zum Zeitpunkt . Die eben erläuterten Funk- tionen der constraints sind in nachfolgender Abbildung 2.1 nochmals graphisch veranschau- licht.Abbildung 2.1: Constraints des zeitkontinuierlichen optimalen SteuerungsproblemsQuelle:[Die 2014, S. 61]Zur Lösung des zeitkontinuierlichen Problems existieren verschiedene Verfahren. Für praxis- relevante Probleme kommen nach [Die 2014] jedoch ausschließlich die sogenannten Direct Methods zum Einsatz. Das Grundprinzip dieser Methoden besteht in der Diskretisierung des zeitkontinuierlichen Problems und anschließender iterativer Lösung (z. B. mit dem Newton- Verfahren) unter Verwendung der KKT (Karush Kuhn Tucker)-Optimalitätsbedingungen. [Die 2014, S. 61ff.] Da diese Arbeit sich lediglich mit der Modellerstellung für Optimalitäts- steuerungsprobleme befasst, sei für weiterführende Informationen zu den Lösungsverfahren sowie zur theoretischen Basis auf [Die 2014] verwiesen.Die Steuerung des solarbetriebenen Kühlungssystems im K-Gebäude der Hochschule Karl- sruhe soll mit Hilfe der Modellprädiktiven Regelung erfolgen. Beim Einsatz von MPC wird das Steuerungsproblem aus (2.1) im laufenden Betrieb der Anlage - auf Basis aktueller Mes- sungen des Systemzustands - immer wieder neu gelöst. Dadurch ist es möglich, Modellunge- nauigkeiten und Störungen teilweise auszugleichen.2.2 ModellanforderungenDie Anforderung der Stetigkeit der Modelle resultiert aus den Lösungsmethoden der Direct Methods. Sowohl für die Formulierung der KKT-Bedingungen als auch für die Verwendung der Gradienten-basierten Verfahren - wie beispielsweise des Newton-Verfahrens - bedarf es der Bildung von Ableitungen. Aufgrund dessen müssen das zeitkontinuierliche Problem und damit auch das verwendete Modell stetig sein.  
Optimale Steuerung und Modellprädiktive Regelung 6 Die Anforderung der geringen Komplexität ergibt sich aus der Arbeitsweise der Modellprä- diktiven Regelung. Da das Steuerungsproblem im laufenden Betrieb immer wieder neu gelöst wird, bedarf es einer möglichst schnellen Lösungsermittlung.Um sicherzustellen, dass die errechneten Steuergrößen auch den gewünschten Effekt auf die Leistungskriterien erzielen, ist ein Modell von angemessener Güte notwendig.Für die beiden letztgenannten Anforderungen gilt es häufig einen sinnvollen Kompromiss zu finden, da eine hohe Modellgüte meist zu Lasten der Komplexität geht und umgekehrt.
