package room_model_backup
  ...
  model Radiator "model for a discretized radiator within a room"
    ...
    parameter Real radiator_element_number=106 "number of normed elements of which the radiator consists";
    parameter Real radiator_tubes_element=2 "number of parallel tubes in one element";
    parameter Integer cv_number = 20 "number of control volumes in which the radiator is discretized";
    parameter Modelica.SIunits.Length radiator_element_length=0.045 "length of one element depending on type (Recknagel 2013/2014: Heizung und Klimatechnik S.815ff.)";
    parameter Modelica.SIunits.Height tube_length_vertical=0.4 "height of one element depending on type (Recknagel 2013/2014: Heizung und Klimatechnik S.815ff.)";
    parameter Modelica.SIunits.Diameter tube_diameter_horizontal=0.05 "diameter of the horizontal tubes depending on type (Recknagel 2013/2014: Heizung und Klimatechnik S.815ff.)";
    parameter Modelica.SIunits.Diameter tube_diameter_vertical=0.0255 "diameter of the vertical tubes depending on type (Recknagel 2013/2014: Heizung und Klimatechnik S.815ff.)";
    parameter Modelica.SIunits.Density rho_water = 1000 "density of water";
    parameter Modelica.SIunits.Mass radiator_element_mass=0.35 "mass within one element of the radiator depending on type (Recknagel 2013/2014: Heizung und Klimatechnik S.815ff.)";

    inner parameter Modelica.SIunits.CoefficientOfHeatTransfer u_radiator = 12.9872 heat transfer coefficient of the radiator";
    inner parameter Modelica.SIunits.SpecificHeatCapacity cp_water = 4182 "specific heat capacity of water";                                 
    CV_Radiator[cv_number] cv_radiator "array of control volumes to discretize the radiator";
    ...
    inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC room_temperature_cv "temperature within the room for the control volume";
    outer Modelica.SIunits.Conversions.NonSIunits.Temperature_degC room_temperature "temperature within the room from the room";

    /** controls **/
    inner Modelica.SIunits.MassFlowRate mdot=inlet.mdot "massflowrate within the radiator";
    Modelica.SIunits.Conversions.NonSIunits.Temperature_degC radiator_inlet = inlet.t "temperature of the inflowing fluid";
  ...
  model Room_radiator_window "model of a room for mpc purpose with JModelica.org"
      ...
      parameter Modelica.SIunits.CoefficientOfHeatTransfer u_glass=2.0 "heat transfer coefficient for glass ";                        
      parameter Modelica.SIunits.CoefficientOfHeatTransfer u_wall=0.612986 "heat transfer coefficient for the walls of the room"; 
      parameter Modelica.SIunits.SpecificHeatCapacity cp_air=1005 "specific heat capacity of air";
      parameter Modelica.SIunits.SpecificHeatCapacity cp_water=4182 "specific heat capacity of water";
      ...
      Modelica.SIunits.Area building_surface "sum of contacting surfaces (walls) with other rooms of the building";
      Modelica.SIunits.Area environment_surface "sum of contacting surfaces (walls) with the environment";
      inner Modelica.SIunits.Area window_surface "sum of contacting surfaces (windows) with the environment";
      Modelica.SIunits.Energy room_u "inner energy of the system room";
      Modelica.SIunits.HeatFlowRate building_qdot  "rate of heat flow with other rooms within the building";    
      Modelica.SIunits.HeatFlowRate environment_qdot "rate of heat flow with the environment";
      Modelica.SIunits.HeatFlowRate environment_qdot_wall "rate of heat flow with the environment through the wall";
      Modelica.SIunits.HeatFlowRate environment_qdot_window "rate of heat flow with the environment through the window";
      Modelica.SIunits.HeatFlowRate qdot_loss "summed up rate of heatflow leaving the system";
      Modelica.SIunits.HeatFlowRate radiator_qdot "heat flow rate at the radiator surfaces streaming into the room";

      /** states x **/
      inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC room_temperature(start=24, fixed=true)
      "temperature within the room (Initially 24 degree celsius)";
  ...
  model SourceTempMass
    ...
    room_model_backup.MassTemperature outlet;
  ...
  equation
    connect(realExpression3.y, Heizkorper.inlet_t);
    connect(realExpression6.y, Heizkorper.inlet_mdot);
    connect(realExpression4.y, Gebaude.inlet);
    connect(realExpression2.y, UmgebungA.inlet);
    connect(realExpression1.y, Sonne.inlet);
    connect(room_radiator_window.inlet_other, Sonne.outlet);
    connect(room_radiator_window.inlet_sun, sourceSun.outlet);
    connect(UmgebungA.outlet, room_radiator_window.inlet_environment);
    connect(Gebaude.outlet, room_radiator_window.inlet_building);
    connect(Heizkorper.outlet, room_radiator_window.inlet_radiator);
    connect(realExpression5.y, sourceSun.inlet);
  end RoomRadiator;
  ...
  model Window
    ...
    outer Modelica.SIunits.Area window_surface "sum of contacting surfaces (windows) with the environment";
    HeatFlow outlet_room;
